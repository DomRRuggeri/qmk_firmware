...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    LCTL(KC_C),   LCTL(KC_V),         TO(5),      LCTL(KC_F), 
    LCTL(KC_GRV), LCTL(KC_F5),        LCTL(KC_T), LCTL(KC_W), 
    LCTL(KC_ENT), TO(3),              TO(2),      OSL(1)
    ),

	[1] = LAYOUT_ortho_4x4(
    KC_TRNS,         LCTL(KC_PGUP), LCTL(KC_PGDN),    RESET, 
    ...
    LGUI(KC_1), LGUI(KC_2), LGUI(KC_3), KC_TRNS
    ),

	[3] = LAYOUT_ortho_4x4(
    TO(0),         LGUI(KC_K),  LGUI(KC_LGUI), LGUI(KC_L), 
    LGUI(KC_LEFT), LGUI(KC_UP), LGUI(KC_DOWN), LGUI(KC_RGHT), 
    LGUI(KC_E),    LCTL(KC_L),  KC_NO,         TG(4), 
    LALT(KC_F4),   KC_NO,       KC_NO,         KC_NO
    ),
